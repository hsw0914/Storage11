...
$board = $db->query("SELECT * FROM `news_community`  WHERE title LIKE ? ORDER BY `idx` DESC LIMIT {$loadStart}, {$perPage}", array( "%".$_GET['search']."%"))->fetchAll();
$boardLength = $db->query("SELECT * FROM `news_community` WHERE title LIKE ?", array( "%".$_GET['search']."%"))->rowCount();
